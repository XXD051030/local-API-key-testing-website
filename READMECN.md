...
### v2.3.3
- 切换 `API Key` 时，现在会自动带出该 key 的默认模型，选完 key 就可以直接开始对话。
- 你为某个 key 最后一次手动选择的模型，现在会被记住并作为这个 key 的默认模型。
- `settings.json` 中的模型预设已改成更清晰的按 key 绑定格式，包含 `keyName` 和 `defaultModel`。
- 在 `Model Presets` 中删除单个模型时，现在会先弹出确认提示。
- 删除单条聊天记录时，现在也会先弹出确认提示，避免误触。
