...
### v2.3.4
- 搜索 provider 的 API Key 输入框现在会立即更新内存里的搜索设置，所以你刚粘贴完 key 就立刻发送消息时，联网搜索也能正常工作。
- 如果模型请求了一个还没配置 API Key 的搜索 provider，应用现在会自动回退到当前已配置可用的 provider，而不会再误报“没有配置搜索 key”。
